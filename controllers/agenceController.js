...
exports.agenceListe = function(req, res) {
    connection.query("SELECT fkuser, age.idagence, age.nomagence FROM users.agence age inner join users.destination des on des.fkagence = age.idagence where des.fkuser = ?", req.session.iduser, function (error, resultSQL) {
        if (error) {
